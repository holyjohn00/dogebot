...
var Bot = function Constructor(settings){
	...
	this.settings.name = this.settings.name || 'Bot';
...
Bot.prototype._onMessage = function(){
	if(this._isAskingChuckNorris){
		this._returnChuckNorrisFacts();

	}
	if(this._isAskingDogebot){
		this._returnAnswer(message);
	}
	if(this._isPingingForSite){
...
Bot.prototype._replyJoke = function (originalMessage){
	...
	automeme.getMeme(function(err, meme) {
	  ...
	  var channelID = self._getChannelById(originalMessage.channelID);
		self.postMessageToChannel(channel.name,meme,{as_user: true});
	});
}
...
Bot.prototype._returnAnswer = function(message){
	...
	wolfram.query(query, function(err,result){
		...
		var channelID = self._getChannelById(originalMessage.channelID);
		self.postMessageToChannel(channel.name,result,{as_user: true});
	});.
}

Bot.prototype._returnChuckNorrisFacts = function(){

	var self = this;
	self.db.get('SELET id, jokes FROM jokes ORDER BY RANDOM() LIMIT 1', function (err, record){
		if(err){
